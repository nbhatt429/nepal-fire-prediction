...
print("Nepal Forest Fire Ensemble Prediction (XGBoost + Transformer)")
print("="*60)
...
for col, default in [("ndvi_30to45",0.30),("ndvi_60to90",0.35),
    ...
    if col not in static_df.columns:
        ...
        static_df[col] = static_df[col].fillna(default)

static_df["dist_water_km"] = static_df.get("dist_water_km", 1.0)
print(f"Grid points: {len(static_df):,}")

# ── Fetch ERA5 historical + forecast for coarse grid ───────────
try:
    ...
    OM_AVAILABLE = False
    print("openmeteo_requests not installed — using fallback weather")
...
def fetch_era5_coarse():
    """Fetch 90-day ERA5 on 0.5-deg coarse grid, return (11200,90,18) array."""
    ...
    for i, (lat, lon) in enumerate(coarse_pts):
        try:
            ...
            times = pd.date_range(
                ...
                inclusive="left")
            df_h = pd.DataFrame(
                {VARIABLES[j]: h.Variables(j).ValuesAsNumpy()
                 for j in range(6)}, index=times)
            ...
            times_fc = pd.date_range(
                ...
                inclusive="left")
            df_fc = pd.DataFrame(
                {VARIABLES[j]: hf.Variables(j).ValuesAsNumpy()
                 for j in range(6)}, index=times_fc)
            ...
            all_hist[(lat,lon)] = combined.values  # (90,18)
    ...
    # Interpolate to 11200 grid points
    coarse_lats = np.array([k[0] for k in all_hist])
    coarse_lons = np.array([k[1] for k in all_hist])
    coarse_vals = np.array(list(all_hist.values()))  # (N,90,18)
    ...
    print(f"ERA5 grid shape: {era5_grid.shape}")
    return era5_grid
...
def build_xgb_features(era5_grid, terrain_df):
    ...
    era5_feats(0,"temperat_max"); era5_feats(1,"temperat_mean"); era5_feats(2,"temperat_min")
    era5_feats(3,"totalpre_max"); era5_feats(4,"totalpre_mean"); era5_feats(5,"totalpre_min")
    era5_feats(12,"surfacep_max"); era5_feats(13,"surfacep_mean"); era5_feats(14,"surfacep_min")
    ...
    era5_feats(15,"volumetr_max"); era5_feats(16,"volumetr_mean"); era5_feats(17,"volumetr_min")
...
# ── Main prediction ────────────────────────────────────────────
print("Fetching ERA5 (historical + forecast)...")
...
if era5_grid is not None:
    ...
    print(f"XGBoost done. Mean prob: {xgb_probs.mean():.3f}")
else:
    print("ERA5 fetch failed — using fallback zeros")
    xgb_probs = np.zeros(len(static_df))

# ── Risk levels ────────────────────────────────────────────────
# Threshold 0.4 = best F1 for early warning
THRESHOLD_VH = 0.7
THRESHOLD_H  = 0.4
THRESHOLD_M  = 0.3

def to_risk(p):
    if p >= THRESHOLD_VH: return "VERY HIGH"
    if p >= THRESHOLD_H:  return "HIGH"
    if p >= THRESHOLD_M:  return "MEDIUM"
    return "LOW"

date_str    = datetime.now().strftime("%Y-%m-%d")
results_df  = static_df[["latitude","longitude"]].copy()
results_df["probability"] = np.round(xgb_probs, 4)
results_df["risk_level"]  = [to_risk(p) for p in xgb_probs]
...
ax.set_title(
    ...
    f"XGBoost Ensemble (73.1% Detection) + Open-Meteo 90-day ERA5",
    fontsize=12,fontweight="bold")
ax.set_xlim(80.0,88.2); ax.set_ylim(26.3,30.5)
ax.grid(True,alpha=0.3)
plt.tight_layout()
plt.savefig(f"fire_risk_map_{date_str}.png",dpi=120,
            bbox_inches="tight",facecolor="white")
print(f"Map saved: fire_risk_map_{date_str}.png")
print("DONE")


with open('/content/drive/MyDrive/predict_ensemble.py', 'w') as f:
    f.write(new_predict)
print("Saved predict_ensemble.py to Drive")
